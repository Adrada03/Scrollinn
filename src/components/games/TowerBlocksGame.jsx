...
const TowerBlocksGame = ({ isActive, onNextGame, onReplay, userId }) => {
  ...
  const { submit, loading: isSubmittingScore, error: submitError, lastResult, xpGained, gameId } = useSubmitScore(userId, GAME_IDS.TowerBlocksGame);
  ...
  return (
    <div className="w-full h-full relative overflow-hidden">
      ...
      {(isPlaying || isEnded || status === GAME_STATES.RESETTING) && (
        <div className="absolute inset-x-0 top-[calc(var(--sat,0px)+5.5rem)] text-center pointer-events-none z-[2]">
          <span
